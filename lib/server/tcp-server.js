...
function Res(socket, ack) {
  ...
  this.end = function(overrideAck) {
    socket.write(VT + (overrideAck || ack).toString() + FS + CR);
  }
}
  ...
  this.server = net.createServer(function(socket) {
    ...
    socket.on('data', function(data) {
      try {
        ...
        if (message.substring(message.length - 2, message.length) == FS + CR || options.skipValidation) {
