...
TcpClient.prototype.connect = function (callback) {
	...
	self.client = net.connect({host: self.host, port: self.port}, function () {
		self.client.on('data', function (data) {
			...
			if (
				self.responseBuffer.substring(
					self.responseBuffer.length - 2,
					self.responseBuffer.length,
				) ==
				FS + CR
			) {
				var ack = self.parser.parse(
					self.responseBuffer.substring(1, self.responseBuffer.length - 2),
				);
