...
segment.prototype.getFieldComponent = function(index, repeatIndex, componetIndex) {
  if (this.fields.length >= index) {
    var field = this.fields[index - 1]
    if (repeatIndex) {
      if (field.value.length >= repeatIndex) {
        if (componetIndex) {
          var item = field.value[repeatIndex - 1];
          if (item.value.length >= componetIndex) {
            return item.value[componetIndex - 1][0].toString(delimiters);
          } else {
            return ""
          }
