...
encoder.prototype.encode = function (msg) {
    ...
    config.segments.forEach(function (segmentConfig) {
        ...
        var prop = segmentConfig.property;
        var segments = id == "MSH"
            ? [msh.header]
            : msg.getSegments(id);

        if (!segments.length) return;
        var data = encodeSegments(msg, segments, segmentConfig);
...
encoder.prototype.decode = function (obj) {
    var config = this.config;
    ...
    config.segments.forEach(function (segment) {
        decodeSegment(msh, obj, config, segment)
    })
...
function encodeSegments(msg, segments, config) {
    var obj;
    if (segments.length > 1) {
        obj = []
        segments.forEach(function (segment) {
            obj.push(encodeSegment(msg, segment, config));
        })
    } else {
        obj = encodeSegment(msg, segments[0], config);
    }
    return obj;
}

function encodeSegment(msg, segment, config) {
    ...
    if (config.segments) {
        ...
        config.segments.forEach(function (subConfig) {
            ...
            var subObj = encodeSegments(msg, subSegments, subConfig);
...
function setValue(msg, segment, obj, value, config) {
    ...
    if (value.field) {
        if (value.repeats) {
            ...
            if (value.type) val = encodeValue(val, value.type)
        }
...
function encodeValue(stringValue, type) {
    if (type == "Number") {
...
function decodeSegment(msh, obj, config, segment) {
...
function pushValueToFields(msh, obj, config, segment, fields, value) {
    var val = get(obj, value.path) || '';
        if (value.field) {
            if (value.repeats) {
                if (value.type) val = val.map(x => decodeValue(x, value.type, config));
            } else {
                if (value.type) val = decodeValue(val, value.type, config);
            }
...
function decodeValue(value, type, config) {
    if (type == "Date") {
